...
let startQueryExecutionRecursivelyCounter = 0;
	...
	return new Promise(function (resolve, reject) {
		const keepCheckingRecursively = async function () {
			try {
				...
				console.log("startQueryExecutionRecursivelyCounter", startQueryExecutionRecursivelyCounter);
				if (retries > 3 && startQueryExecutionRecursivelyCounter < 3) {
					...
					startQueryExecutionRecursivelyCounter++;
